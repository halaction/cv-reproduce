...
The current codebase includes four ablation paths exposed through the shared training script: a convolutional replacement for the patch stem, a non-linear channel mixer replacing the original pointwise convolution, an identity-based no-channel-mixing variant, and a reduced-data protocol spanning multiple model families.

\paragraph{Hardware and runtime context.}
The reported runs in the current Weights \& Biases export were executed on CUDA and logged through the shared training script. Earlier debugging was also performed on CPU during development, but those runs are not used in the quantitative comparison reported here. To preserve compute, training was stopped manually once test accuracy had not improved for roughly more than three epochs. As a result, different architectures were allowed to stop at different epochs after reaching a clear plateau.
